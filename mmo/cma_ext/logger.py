...
class CMADataLogger(interfaces.BaseDataLogger):
    ...
    :See: `disp` (), `plot` ()
    """
    default_prefix = 'mmo/' + '.outcmaes' + os.sep
...
#                                str(type(es)))
    ...
    def initialize(self, modulo=None):
        ...
        try:
            with open(fn, 'w') as f:
                fwrite('% # columns="iteration, evaluation, sigma, axis ratio, ' +
        ...
        try:
            with open(fn, 'w') as f:
                fwrite('% # columns="iteration, evaluation, sigma, ' +
                        'max axis length, ' +
        ...
        try:
            with open(fn, 'w') as f:
                fwrite('% # columns="iteration, evaluation, min max(neg(.)) min(pos(.))' +
        ...
        try:
            with open(fn, 'w') as f:
                fwrite('% # columns="iteration, evaluation, sigma, void, void, ' +
        ...
        try:
            with open(fn, 'w') as f:
                fwrite('% # columns="iteration, evaluation, void, void, void, xmean", ' +
                        strseedtime +
                        ', ' + self.persistent_communication_dict.as_python_tag
                        )
                fwrite(' # scaling_of_variables: ')  # todo: put as python tag
                if np.size(es.gp.scales) > 1:
                    fwrite(' '.join(map(str, es.gp.scales)))
                else:
                    fwrite(str(es.gp.scales))
                fwrite(', typical_x: ')
                if np.size(es.gp.typical_x) > 1:
                    fwrite(' '.join(map(str, es.gp.typical_x)))
                else:
                    fwrite(str(es.gp.typical_x))
                fwrite('\n')
        except (IOError, OSError):
            print('could not open/write file ' + fn)
        ...
        try:
            with open(fn, 'w') as f:
                fwrite('% # columns="iter, evals, sigma, 0, fitness, xbest" ' +
                        strseedtime +
                        ', ' + self.persistent_communication_dict.as_python_tag +
                        '\n')
        except (IOError, OSError):
