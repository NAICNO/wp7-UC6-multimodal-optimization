...
| `benchmarks/CEC2013/` | CEC2013 benchmark suite (git submodule) |
...
├── benchmarks/CEC2013/                 # Benchmark submodule
...
cd benchmarks/CEC2013/python3 && python setup.py install && cd ../../..
```
...
```bash
tmux new-session -d -s benchmark 'cd ~/multi-modal-optimization && source activate-mmo.sh && \
...
| Issue | Solution |
|-------|----------|
| `cec2013` import error | Run `./fix-cec2013.sh` or rebuild: `cd benchmarks/CEC2013/python3 && python setup.py install` |
| `ModuleNotFoundError: No module named 'mmo'` | Activate environment: `source activate-mmo.sh` |
| CEC2013 submodule empty | `git submodule update --init --recursive` |
...
- CEC2013 benchmarks (BSD 2-Clause) — git submodule in `benchmarks/CEC2013/`
